...
const int ttotal = 14;                                 // Tiempo total (segundos) de captura de datos
...
// MQTT (Adafruit IO)
int indicador_IO = 0;                                  // Asociada a los indicadores de Adafruit
String icon_IO = "";                                   // Asociada al icono de warning si hay epilepsia
String textIO = "";                                    // Asociada al registro de movimientos
AdafruitIO_Feed *indicadorIO = io.feed("indicadorIO"); // Preparamos todas las feeds
AdafruitIO_Feed *iconIO = io.feed("iconIO");
AdafruitIO_Feed *text_IO = io.feed("text_IO");
bool epil = false;                       // Para saber si enviar o no, dato correspondiente a iconIO (lo almacenamos en la memoria RTC para no resetearse tras el DeepSleep)
...
void loop() {
  ...
  if (interruptCounter > 0) {       // Entrada a la interrupcion
...
void enviarMQTT(){ // Funcion a la que se entra cada 2 segundos para enviar los datos
  ...
  // Indicadores:
  if (prediccion_b == "p_andar"){
    indicador_IO = 1;
  }
  else if (prediccion_b == "p_epil1"){
    indicador_IO = 2;
  }
  else if (prediccion_b == "p_epil2"){
    indicador_IO = 3;
  }
  else if (prediccion_b == "p_sent1"){
    indicador_IO = 4;
  }
  else {
    indicador_IO = 5;
  }
  indicadorIO->save(indicador_IO);
